...
contextBridge.exposeInMainWorld('journalAPI', {
  ...
  restoreTyping: (): Promise<{ logged: boolean }> => ipcRenderer.invoke('window:restore-typing'),
  setAlwaysOnTop: (enabled: boolean): Promise<void> => ipcRenderer.invoke('window:always-on-top', enabled),
