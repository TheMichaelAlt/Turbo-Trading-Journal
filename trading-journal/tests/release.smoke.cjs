...
async function main() {
  ...
  try {
    ...
    await page.getByLabel('Trade CSV', { exact: true }).setInputFiles({ name: 'smoke.csv', mimeType: 'text/csv', buffer: Buffer.from('Symbol,PnL,Notes\nMNQ,12.5,Packaged import notes') })
    ...
    assert.ok(persisted.trades.some(t => t.values.notes === 'Packaged import notes' && t.values.pnl === 12.5))
