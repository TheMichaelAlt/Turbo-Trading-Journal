...
import { activeFields, fieldError, isBlank, isComplete, isNumeric, isTextCharacteristic, today, tradeIssues, type Field, type JournalData, type Trade, type Values } from '../lib/model'
    ...
    {minimalist ? <form className="quick-entry" noValidate onSubmit={save} aria-label="Minimalist trade entry">
      ...
      <div className="quick-entry-body">
        <div className="quick-fields" ref={scrollRow} role="region" aria-label="Trade characteristics" tabIndex={0}>
          {[...required, ...optional].map(field => <FieldInput key={field.id} compact field={field} value={values[field.id]} onChange={value => change(field.id, value)} error={errors[field.id]} />)}
        </div>
