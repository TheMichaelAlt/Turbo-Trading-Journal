...
import { activeFields, isComplete, isBlank, type JournalData, type Trade } from '../lib/model'
...
export default function TradeTable({ data, onEdit, onDelete, onNew }: { data: JournalData; onEdit: (t: Trade) => void; onDelete: (id: string) => Promise<boolean>; onNew: () => void }) {
  ...
  const [columns, setColumns] = useState(['date', 'timeIn', 'timeOut', 'direction', 'contract', 'size', 'pnl', 'account', 'strategy', 'setupGrade'])
  ...
  const completed = data.trades.filter(t => isComplete(t, data.fields)), fields = activeFields(data.fields)
  ...
  const visible = fields.filter(f => columns.includes(f.id)), pages = Math.max(1, Math.ceil(trades.length / 25)), currentPage = Math.min(page, pages - 1)
  const exportCsv = () => {
