...
import { DEFAULT_TRADING_DAY_END } from '../lib/tradingDay'
...
export default function Dashboard({ data, onNew, onDemo, update }: { data: JournalData; onNew: () => void; onDemo: () => void; update: (change: (data: JournalData) => JournalData) => Promise<boolean> }) {
  const filters = data.lastFilters?.dashboard?.filters || emptyFilters()
  const setFilters = (filters: ReturnType<typeof emptyFilters>) => { void update(current => ({ ...current, lastFilters: { ...current.lastFilters, dashboard: { filters } } })) }
  const display = data.pnlDisplay || 'dollars'
  return <>
...
function DashboardView({ data: sourceData, onNew, onDemo, unit, filters, setFilters }: { data: JournalData; onNew: () => void; onDemo: () => void; unit: 'dollars' | 'points'; filters: ReturnType<typeof emptyFilters>; setFilters: (value: ReturnType<typeof emptyFilters>) => void }) {
  const data = { ...sourceData, currency: unit === 'points' ? 'PTS' : sourceData.currency }
  ...
  const matched = filterTrades(completed, filters, data.tradingDayEnd ?? DEFAULT_TRADING_DAY_END), trades = pnlTrades(matched, unit, data), stats = metrics(trades)
  ...
  return <>
    ...
    <p className="help-text">Date filters use trading days. Entries after {data.tradingDayEnd ?? DEFAULT_TRADING_DAY_END} belong to the next day.</p>
    <PerformanceCalendar tradingDayEnd={data.tradingDayEnd} trades={trades} currency={data.currency} focusMonth={(filters.to || filters.from).slice(0, 7) || undefined} />
    {stats.excluded > 0 && <div className="notice">{stats.excluded} completed trade(s) are excluded from these PnL statistics because their PnL is missing or their contract multiplier is unknown. Set the multiplier in Settings to include conversions. Their original records remain intact.</div>}
    {!completed.length ? <div className="panel"><Empty title="Your edge starts here." action={<div className="button-row"><button className="primary" onClick={onNew}>Log your first trade <ArrowUpRight size={16} /></button>{!totalDemo && <button className="secondary" onClick={onDemo}>Explore sample data</button>}</div>}>Complete a trade to see your equity curve, strategy performance, and automatic characteristic breakdowns.</Empty></div> : !trades.length ? <div className="panel"><Empty title="No trades match these filters." action={<button className="secondary" onClick={() => setFilters(emptyFilters())}>Clear filters</button>}>Try a wider date range or a different combination.</Empty></div> : <>
      <div className="chart-grid"><section className="panel"><div className="panel-heading"><div><h2>Equity curve</h2><p>Your cumulative performance, trade by trade</p></div><span className="legend"><i />Net PnL</span></div><EquityChart data={stats.curve} currency={data.currency} /></section><section className="panel"><div className="panel-heading"><div><h2>Daily performance</h2><p>Net PnL by trading day</p></div><span className="tag">{dailyResults(trades, data.tradingDayEnd).length} days</span></div><DailyChart data={dailyResults(trades, data.tradingDayEnd)} currency={data.currency} /><div className="chart-caption">Hover over a bar for the date and PnL</div></section></div>
